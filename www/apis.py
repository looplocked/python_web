...
class Page(object):
    ...
    def __init__(self, item_count, page_index=1, page_size=10):
        ...
        10
        '''
        self.item_count = item_count    # 所有博文数量
        self.page_size = page_size    # 每页显示博文数量
        self.page_count = item_count // page_size + (1 if item_count % page_size > 0 else 0)    # 总页数
        if (item_count == 0) or (page_index > self.page_count):    # 如果数据库没有博客或全部博客总页数不足
            self.offset = 0
            self.limit = 0
            self.page_index = 1
        else:
            self.page_index = page_index
            self.offset = self.page_size * (page_index - 1)    # 从上一页开始往前的所有博文数
